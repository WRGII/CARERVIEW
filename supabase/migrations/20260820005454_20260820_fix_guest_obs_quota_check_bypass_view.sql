/*
# Fix guest observation submission: quota check permission denied

## Problem
`cv_submit_guest_observation` queries the `cv_v_team_remaining` view to check
the team's observation quota. That view has `security_invoker=true` and calls
`is_team_member()` internally. Recent security hardening revoked all EXECUTE
grants from `is_team_member`, so when an unauthenticated guest (anon role)
triggers this function, the view fails with:
  "permission denied for function is_team_member"

## Fix
Replace the view lookup with an inline query that directly calculates the
team's remaining observation quota. The function already validated the token
and knows the team_id, so there is no need for an `is_team_member` check —
the token itself authorises the guest to submit to that team.

## Security
- Function remains SECURITY DEFINER, same owner (postgres).
- No new grants; no policy changes.
- The inline query only reads the specific team identified by the validated
  guest token — no broader access.
- All other logic (token validation, score validation, insert, consume)
  is unchanged.
*/
