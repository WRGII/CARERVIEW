/*
# Fix search_path on guest token functions

## Problem
Three SECURITY DEFINER functions that call pgcrypto functions (`gen_random_bytes`, `digest`)
had `search_path TO 'public'` only, missing the `'extensions'` schema where pgcrypto is installed.
This caused "function gen_random_bytes(integer) does not exist" errors at runtime.

## Changes
Modified the `SET search_path` clause on these three functions to include `'extensions'`:
- `cv_create_guest_token(uuid,text,text,text,text)` — generates random tokens via `gen_random_bytes(32)` and hashes via `digest()`
- `cv_peek_guest_token(text)` — validates a guest token by hashing it with `digest()`
- `cv_submit_guest_observation(text,text,text,date,text,text,jsonb)` — validates and consumes a guest token via `digest()`

## Security
- No RLS or policy changes.
- Functions remain SECURITY DEFINER with the same logic; only the search_path clause changes.
- Adding `'extensions'` to search_path is the same pattern already used by `cv_create_invite`, `cv_accept_invite`, `cv_peek_invite`, and others.

## Notes
1. The function bodies are unchanged — only the `SET search_path` clause is updated.
2. Idempotent: uses `CREATE OR REPLACE FUNCTION`.
*/
