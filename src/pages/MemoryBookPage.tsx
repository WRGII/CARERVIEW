...
import { ChevronDown, CircleCheck as CheckCircleIcon, ArrowRight, BookOpen, Chrome as Home, User, Users, Stethoscope, Heart, Activity, Shield, DollarSign, Zap, Car, Repeat } from "lucide-react";
...
export default function MemoryBookPage() {
  ...
  const benefits = [
    { title: t("mb_page.benefit1_title"), body: t("mb_page.benefit1_body") },
    { title: t("mb_page.benefit2_title"), body: t("mb_page.benefit2_body") },
    { title: t("mb_page.benefit3_title"), body: t("mb_page.benefit3_body") },
    { title: t("mb_page.benefit4_title"), body: t("mb_page.benefit4_body") },
    { title: t("mb_page.benefit5_title"), body: t("mb_page.benefit5_body") },
  ];

  const memoryBookFeatures = [
    {
      icon: User,
    ...
    },
    {
      ...
      title: t("mb_page.feat_daily_title"),
      body: t("mb_page.feat_daily_body"),
      fields: [
        t("mb_page.feat_daily_f1"),
        t("mb_page.feat_daily_f2"),
        t("mb_page.feat_daily_f3"),
        t("mb_page.feat_daily_f4"),
        t("mb_page.feat_daily_f5"),
      ],
    },
    {
      icon: Heart,
  ...
  const careBookFeatures = [
    {
      icon: Shield,
      color: "bg-blue-50 text-blue-600 border-blue-100",
      title: t("mb_page.feat_insurance_title"),
      body: t("mb_page.feat_insurance_body"),
      fields: [
        t("mb_page.feat_insurance_f1"),
        t("mb_page.feat_insurance_f2"),
        t("mb_page.feat_insurance_f3"),
        t("mb_page.feat_insurance_f4"),
        t("mb_page.feat_insurance_f5"),
      ],
    },
    {
      icon: DollarSign,
      color: "bg-amber-50 text-amber-600 border-amber-100",
      title: t("mb_page.feat_finances_title"),
      body: t("mb_page.feat_finances_body"),
      fields: [
        t("mb_page.feat_finances_f1"),
        t("mb_page.feat_finances_f2"),
        t("mb_page.feat_finances_f3"),
        t("mb_page.feat_finances_f4"),
      ],
    },
    {
      icon: Zap,
      color: "bg-orange-50 text-orange-500 border-orange-100",
      title: t("mb_page.feat_providers_title"),
      body: t("mb_page.feat_providers_body"),
      fields: [
        t("mb_page.feat_providers_f1"),
        t("mb_page.feat_providers_f2"),
        t("mb_page.feat_providers_f3"),
        t("mb_page.feat_providers_f4"),
      ],
    },
    {
      icon: Car,
      color: "bg-sky-50 text-sky-600 border-sky-100",
      title: t("mb_page.feat_vehicles_title"),
      body: t("mb_page.feat_vehicles_body"),
      fields: [
        t("mb_page.feat_vehicles_f1"),
        t("mb_page.feat_vehicles_f2"),
        t("mb_page.feat_vehicles_f3"),
        t("mb_page.feat_vehicles_f4"),
      ],
    },
    {
      icon: Repeat,
      color: "bg-violet-50 text-violet-500 border-violet-100",
      title: t("mb_page.feat_subs_title"),
      body: t("mb_page.feat_subs_body"),
      fields: [
        t("mb_page.feat_subs_f1"),
        t("mb_page.feat_subs_f2"),
        t("mb_page.feat_subs_f3"),
        t("mb_page.feat_subs_f4"),
      ],
    },
  ];
  ...
  const faqs = [
    ...
    { q: t("mb_page.faq8_q"), a: t("mb_page.faq8_a") },
  ];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-warm-white via-white to-peach-blush/20">
      <PageSEO
        title="Memory Book & Care Book — CarerView"
        description="CarerView's Memory Book captures who your loved one is and how to care for them. The Care Book keeps every household detail organised. Together they cover every dimension of confident caregiving."
        canonical={`${SITE_URL}/memory-book`}
        keywords="memory book for elderly care, caregiver information profile, care handoff tool, family caregiver guide, loved one care profile, dementia care notes, caregiver handover, household care record, care book, insurance caregiving, daily living assessment"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {benefits.map((b) => (
              <Card key={b.title} className="border-0 shadow-md hover:shadow-lg transition-shadow duration-300 bg-warm-white">
                <CardContent className="p-6 text-center">
                  <h3 className="text-base font-semibold text-slate-800 mb-2">{b.title}</h3>
        ...
        <div className="py-14">
          ...
          {/* MEMORY BOOK PILLAR */}
          <PillarSection
            label={t("mb_page.pillar_mb_label")}
            heading={t("mb_page.pillar_mb_heading")}
            body={t("mb_page.pillar_mb_body")}
            accentClass="border-cyan-300 text-cyan-700 bg-cyan-50"
            headingAccent="text-cyan-700"
            features={memoryBookFeatures}
          />
          ...
          {/* CARE BOOK PILLAR */}
          <PillarSection
            label={t("mb_page.pillar_cb_label")}
            heading={t("mb_page.pillar_cb_heading")}
            body={t("mb_page.pillar_cb_body")}
            accentClass="border-teal-300 text-teal-700 bg-teal-50"
            headingAccent="text-teal-700"
            features={careBookFeatures}
          />
        </div>
...
function PillarSection({ label, heading, body, accentClass, headingAccent, features }: PillarSectionProps) {
  return (
    <div>
      {/* Pillar header */}
      <div className="flex flex-col sm:flex-row sm:items-end gap-4 mb-10">
        <div className="flex-1">
          <span className={`inline-block text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full border mb-3 ${accentClass}`}>
            {label}
          </span>
          <h3 className={`text-3xl font-bold mb-2 ${headingAccent}`}>{heading}</h3>
          <p className="text-lg text-slate-600 max-w-2xl leading-relaxed">{body}</p>
        </div>
      </div>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {features.map((feat) => {
          ...
          return (
            <div
              key={feat.title}
              className="rounded-2xl border border-slate-200 bg-white shadow-sm hover:shadow-md transition-shadow duration-200 overflow-hidden flex flex-col"
            >
              {/* Card header */}
              <div className="px-6 pt-6 pb-4 flex items-start gap-3">
                <div className={`p-2.5 rounded-xl border ${feat.color} shrink-0`}>
                  <Icon className="w-5 h-5" />
                </div>
                <div>
                  <h4 className="font-semibold text-slate-800 text-sm leading-snug">{feat.title}</h4>
                  <p className="text-xs text-slate-500 mt-1 leading-relaxed">{feat.body}</p>
                </div>
              </div>
                ...
                <ul className="space-y-2">
                  {feat.fields.map((field) => (
                    <li key={field} className="flex items-start gap-2.5">
                      <CheckCircleIcon className="w-4 h-4 text-cyan-500 mt-0.5 shrink-0" />
        ...
        <div className="bg-slate-100 border-b border-slate-200 px-4 py-3 flex items-center gap-2">
          ...
          <div className="flex-1 mx-4">
            <div className="bg-white rounded-md px-3 py-1 text-xs text-slate-400 border border-slate-200 max-w-xs mx-auto text-center">
              carerview.app/caregiver/memory-book
            </div>
          </div>
        </div>
        ...
        <div className="border-b border-slate-200 bg-white">
          <div className="flex overflow-x-auto">
            {[
              ...
              { label: t("mb_page.mock_tab_daily"), active: false },
              { label: t("mb_page.mock_tab_household"), active: false },
            ].map((tab) => (
              <div
                key={tab.label}
        ...
        <div className="p-6 md:p-8 bg-slate-50/40">
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
            ...
            <div className="mt-6 pt-5 border-t border-slate-100">
              ...
              <p className="text-sm text-slate-700 leading-relaxed">{t("mb_page.mock_about_text")}</p>
            </div>
          </div>
