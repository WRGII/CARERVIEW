...
export default function ObservationEditPage() {
  ...
  if (obsError)
    return (
      <ErrorMessage
        message={`Error loading observation: ${String(
          (obsError as any)?.message || obsError
        )}`}
      />
    );
  ...
  const isValidType =
    formType === "ADL" || formType === "IADL" || formType === "COMPREHENSIVE";
  if (!isValidType) {
    return (
      <ErrorMessage
        message={`Observation has invalid form type: "${String(formType)}"`}
      />
    );
  }
        ...
        fallback={
          <div className="rounded-md border border-peach-blush bg-peach-blush/20 p-4 text-slate-800">
            ...
            <div className="text-sm">
              Please refresh the page. If the error persists, there may be an
              issue in ObservationForm or its hooks.
            </div>
          </div>
        }
      >
